...
import { signUpWithEmailAndPassword } from "../../api/supabase/supabase-utils/user.js";
...
export default function SignUp(props) {
  ...
  const validateInputs = () => {
    ...
    if (isValid) {
      handleSignUpWithEmail(email.value, password.value, name.value);
    }
  };
  ...
  async function handleSignUpWithEmail(email, password, uname) {
    try {
      const successSignUp = await signUpWithEmailAndPassword(
        email,
        password,
        uname,
      );
  ...
  return (
    <>
      <CustomizedSnackbars
        openState={displaySnack}
        setOpen={setDisplaySnack}
        onClose={handleClose}
      />
      <AppTheme {...props}>
        <CssBaseline enableColorScheme />
        <ColorModeSelect
          sx={{ position: "fixed", top: "8rem", right: "1rem" }}
        />
