export default function AsideComponent({onAdded, yourProjects, onVisiblePage}) {
    return (
        <aside className="p-2 mr-12 text-gray-100 font-bold bg-black bg-opacity-95 rounded-tr-lg min-w-72 max-w-72">
            ...
            <p className="ml-6">
                <button className="p-2 mb-2 rounded-lg items-center text-gray-400 bg-gray-500 bg-opacity-25 hover:bg-opacity-35 hover:text-gray-200 text-sm" onClick={() => onAdded(true)}>
            ...
            <section className="ml-6 min-w-0 min-h-0 m-0">
                <ul>
                    {yourProjects.length > 0 && yourProjects.map((item) => <li key={`${item.titleEntered}_${item.dateEntered}`} className="text-gray-400 hover:bg-gray-800 hover:text-gray-200 mr-4">
